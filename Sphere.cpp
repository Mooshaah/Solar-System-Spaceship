...
//    float distance = sqrt(pow(sphere1.x - sphere2.x, 2) +
//        pow(sphere1.y - sphere2.y, 2) +
//        pow(sphere1.z - sphere2.z, 2));
//
//    // Check for collision
...
bool  Sphere::isColliding(const glm::vec3& object1Pos, const glm::vec3& object2Pos,
    float object1Radius, float object2Radius) {
    glm::vec3 centerDistance = object2Pos - object1Pos;
    float squaredDistance = glm::dot(centerDistance, centerDistance);
    // Combine squared radii for squared collision threshold 
    float combinedRadiusSquared = (object1Radius + object2Radius) * (object1Radius + object2Radius);

    // Check if the squared distance is less than the combined squared radius
